...
const CommercialAutoForm = () => {
    ...
    const storeWorkersCompData = JSON.parse(sessionStorage.getItem("storeWorkersCompData"));
    const storedLeads = JSON.parse(sessionStorage.getItem("lead"));
    const getCommercialAutoData = () => {
       let storedData =  JSON.parse(sessionStorage.getItem("commercialAutoStoredData")) || {};
       return storedData;
    }

    // setting for update and edit
    const [isEditing, setIsEditing] = useState(() => getCommercialAutoData()?.isUpdate == true ? false : true);
    const [isUpdate, setIsUpdate] = useState(() => getCommercialAutoData()?.isUpdate || false);

    // setting Fein and SSN values
    const [fein, setFein] = useState(() => storeWorkersCompData?.feinValue || "");
    const [ssn, setSsn] = useState(() => storeWorkersCompData?.ssnValue || "");

    const [isFeinDisabled, SetIsFeinDisabled] = useState(() => fein == 0  ? false : true);
    const [isSsnDisabled, SetIsSsnDisabled] = useState(() => ssn == 0  ? false : true);

    //setting for dynamic vehicle information
    const [vehicleInformation, setVehicleInformation] = useState(() =>
    getCommercialAutoData()?.vehicleInformation || [{}]);
    const [firstVehicleInformation, setFirstVehicleInformation] = useState(() => getCommercialAutoData()?.firstVehicleInformation || {});

    const allVehicleInformation = [firstVehicleInformation, ...vehicleInformation];
    const [allVehicleInformationData, setAllVehicleInformationData] = useState(() => getCommercialAutoData()?.allVehicleInformation || [])
    const handleAddVehicle = () => {
        setVehicleInformation([...vehicleInformation, {}]); //Add a new vehicle object
    };
    const handleRemoveVehicle = (indexToRemove) => {
        setVehicleInformation(vehicleInformation.filter((_, index) => index !== indexToRemove));
    };
    ...
    const [numberOfDriverToBeInsured, setNumberOfDriverToBeInsured] = useState(() => getCommercialAutoData()?.driverInformation || [{}]);
    const [driverInformation, setDriverInformation] = useState(() => getCommercialAutoData()?.driverInformation || [{}] );
    const [driverQuantity, setDriverQuantity] = useState(() => getCommercialAutoData()?.driverQuantity || 0);
    const [selectedDateOfBirth, setSelectedDateOfBirth] = useState([new Date()]);
    const handleNumberOfDriverToBeInsured = (e)=>{
        const numberOfDriver = e.target.value;
        setDriverQuantity(numberOfDriver);
        const driverInformationArr = Array.from({ length: numberOfDriver }, () => ({}));
    ...
    const handleRemoveDriverInformation = (indexToRemove) => {
        setNumberOfDriverToBeInsured(numberOfDriverToBeInsured.filter((_, index) => index !== indexToRemove));
    };
   ...
   const [garageAddress, setGarageAddress] = useState(() => getCommercialAutoData()?.garageAddress || "");
    ...
    const [isVehiceMaintenanceProgramChecked, setIsVehiceMaintenanceProgramChecked] = useState(() => getCommercialAutoData()?.vehicle_maintenance_program || false);
    const [isVehicleCustomizedChecked, setIsVehicleCustomizedChecked] = useState(() => getCommercialAutoData()?.is_vehicle_customized || false);
    const [isVehicleOwnedProspect, setIsVehicleOwnedProspect] = useState(() => getCommercialAutoData()?.is_vehicle_owned_by_prospect || false);
    const [isDeclinedCanceledNonRenewed, setIsDeclinedCanceledNonRenewed] = useState(() => getCommercialAutoData()?.declined_canceled_nonrenew_policy || false);
    const [isProspectLoss, setIsProspectLoss] = useState(() => getCommercialAutoData()?.prospect_loss || false);
    const [isVehicleUsedTowing, setIsVehicleUsedTowing] = useState(() => getCommercialAutoData()?.vehicle_use_for_towing || false);
    const [vehicleMaintenanceDescription, setVehicleMaintenanceDescription] = useState(() => getCommercialAutoData()?.vehicle_maintenace_description || "");
    const [vehicleCustomizedDescription, setVehicleCustomizedDescription] = useState(() => getCommercialAutoData()?.vehicle_customized_description || "");
    ...
    const [isHaveLossChecked, setIsHaveLossChecked] = useState(() => getCommercialAutoData()?.isHaveLossChecked || false);
    const [dateOfClaim, setDateOfClaim] = useState(() => getCommercialAutoData()?.dateOfClaim ?  new Date(getCommercialAutoData()?.dateOfClaim): new Date());
    const [lossAmount, setLossAmount] = useState(() => getCommercialAutoData()?.lossAmount || "");
    ...
    const [haveLossDateOption, setHaveLossDateOption] = useState(() => getCommercialAutoData()?.haveLossDateOption || 1);
    ...
    let workersCompensationOption = crossSellArray.map(
        ({ value, label }) => ({
            value,
            label,
        })
    ...
    const [crossSell, setCrossSell] = useState(() => getCommercialAutoData()?.crossSell || []);
    ...
    const [expirationOfAuto, setExpirationOfAuto] = useState(() => getCommercialAutoData()?.expirationOfAuto ? new Date(getCommercialAutoData()?.expirationOfAuto): new Date());
    const [priorCarrier, setPriorCarrier] = useState(() => getCommercialAutoData()?.priorCarrier || "");
    ...
    const commercialAutoFormData = {
        ...
        vehicle_maintenace_description: vehicleMaintenanceDescription ? vehicleMaintenanceDescription : "null",
        vehicle_customized_description: vehicleCustomizedDescription ? vehicleCustomizedDescription : "null",
    ...
    return (
        <>
                  ...
                  rowContent={[
                    <Column
                        ...
                        colContent={
                            ...
                            <InputMask
                                ...
                                onChange={(e) => handleFirstVehicleInformationInput("year", e.target.value)}
                                disabled={!isEditing}
                    ...
                    colContent={
                        ...
                        <Form.Control
                           type="text"
                           placeholder="Make"
                           onChange={(e) => handleFirstVehicleInformationInput("make", e.target.value)}
                           disabled={!isEditing}
                    ...
                    rowContent={[
                        <Column
                            ...
                            colContent={
                                ...
                                <Form.Control
                                 type="text"
                                 placeholder="Model"
                                 onChange={(e) => handleFirstVehicleInformationInput("model", e.target.value)}
                                 disabled={!isEditing}
                        ...
                        colContent={
                            ...
                            <Form.Control
                             type="text"
                             placeholder="VIN"
                             onChange={(e) => handleFirstVehicleInformationInput("vin", e.target.value)}
                             disabled={!isEditing}
                 ...
                 rowContent={[
                    ...
                    colContent={
                        ...
                        <Form.Control
                         ...
                         onChange={(e) => handleFirstVehicleInformationInput("radius", e.target.value)}
                         disabled={!isEditing}
                     ...
                     colContent={
                        <>
                         ...
                         <NumericFormat
                           ...
                           allowNegative={false}
                           placeholder="$0.00"
                           onChange={(e) => handleFirstVehicleInformationInput("cost", e.target.value)}
                           disabled={!isEditing}
                     ...
                     rowContent={[
                       <Column
                           ...
                           colContent={
                               ...
                               <InputMask
                                   ...
                                   className="form-control"
                                   placeholder="Year"
                                   onChange={(e) => handleVehicleInformationInput(index, "year", e.target.value)}
                                   disabled={!isEditing}
                                   value={vehicleInformation[index]?.year}
                               />
                               </>
                            }
                       />,
                       ...
                       colContent={
                           ...
                           <Form.Control
                              type="text"
                              placeholder="Make"
                              onChange={(e) => handleVehicleInformationInput(index, "make", e.target.value)}
                              disabled={!isEditing}
                              value={vehicleInformation[index]?.make}
                           />
                           </>
                       }
                       />
                       ...
                       rowContent={[
                           <Column
                               ...
                               colContent={
                                   ...
                                   <Form.Control
                                    type="text"
                                    placeholder="Model"
                                    onChange={(e) => handleVehicleInformationInput(index, "model", e.target.value)}
                                    disabled={!isEditing}
                                    value={vehicleInformation[index]?.model}
                                   />
                                   </>
                                }
                           />,
                           ...
                           colContent={
                               ...
                               <Form.Control
                                type="text"
                                placeholder="VIN"
                                onChange={(e) => handleVehicleInformationInput(index, "vin", e.target.value)}
                                disabled={!isEditing}
                                value={vehicleInformation[index]?.vin}
                               />
                               </>
                           }
                           />
                    ...
                    rowContent={[
                       ...
                       colContent={
                           ...
                           <Form.Control
                            type="text"
                            placeholder="Radius Mileage"
                            onChange={(e) => handleVehicleInformationInput(index, "radius", e.target.value)}
                            disabled={!isEditing}
                            value={vehicleInformation[index]?.radius}
                           />
                           </>
                       }
                       />,
                        ...
                        colContent={
                           <>
                            ...
                            <NumericFormat
                              ...
                              allowNegative={false}
                              placeholder="$0.00"
                              onChange={(e) => handleVehicleInformationInput(index, "cost", e.target.value)}
                              disabled={!isEditing}
                              value={vehicleInformation[index]?.cost}
                            />
                           </>
                        }
                       />
            ...
            {numberOfDriverToBeInsured.map((driver, index) => (
                         ...
                         <Column
                            ...
                            colContent={
                             <>
                               <Button
                                 variant="danger"
                                 onClick={() => handleRemoveDriverInformation(index)}
                                 disabled={!isEditing}
                             ...
                             colContent={
                                 <>
                                     ...
                                     <Form.Control
                                      ...
                                      onChange={(e) => handlDriverInformationInput(index, "firstName", e.target.value)}
                                      disabled={!isEditing}
                                      value={numberOfDriverToBeInsured[index]?.firstName}
                                     />
                         ...
                         <Column
                            ...
                            colContent={
                              ...
                              <Form.Control
                               ...
                               onChange={(e) => handlDriverInformationInput(index, "lastname", e.target.value)}
                               disabled={!isEditing}
                               value={numberOfDriverToBeInsured[index]?.lastname}
                              />
                             ...
                             colContent={
                                 <>
                                     ...
                                     <DatePicker
                                      ...
                                      selected={numberOfDriverToBeInsured[index]?.date_of_birth  ? new Date(numberOfDriverToBeInsured[index]?.date_of_birth) : new Date()}
                                      onChange={(e) => handlDriverInformationInput(index, "date_of_birth", e)}
                                      disabled={!isEditing}
                                     />
                                 </>
                             }
                         ...
                         <Column
                            ...
                            colContent={
                              ...
                              <Select
                                 ...
                                 onChange={(e) => {
                                    handlDriverInformationInput(index, "martial_status", {value:e.value, label:e.label});
                                    handleMaritalStatusChange(e, index);
                                }}
                                value={numberOfDriverToBeInsured[index]?.martial_status}
                  ...
                  />
                  {(selectedMaritalStatus[index] === "married" ||  numberOfDriverToBeInsured[index]?.martial_status?.value === "married") && (
                           <Row
                           classValue="mb-3"
                           rowContent={[
                                  <Column
                                      ...
                                      colContent={
                                        <>
                                           ...
                                           <Form.Control
                                            ...
                                            onChange={(e) => handlDriverInformationInput(index, "spouse_name", e.target.value)}
                                            disabled={!isEditing}
                                            value={numberOfDriverToBeInsured[index]?.spouse_name}
                                        />
                                        </>
                                      }
                                  />
                    ...
                    rowContent={[
                              ...
                              <>
                                  ...
                                  <Form.Control
                                   ...
                                   placeholder="Driver License Number"
                                      onChange={(e) => handlDriverInformationInput(index, "driver_license_number", e.target.value)}
                                      disabled={!isEditing}
                                      value={numberOfDriverToBeInsured[index]?.driver_license_number}
                                  />
                              </>
                          }
                      />,
                      <Column
                         ...
                         colContent={
                           ...
                           <Form.Control
                            type="number"
                            onChange={(e) => handlDriverInformationInput(index, "years_driving_experience", e.target.value)}
                            disabled={!isEditing}
                            value={numberOfDriverToBeInsured[index]?.years_driving_experience}
                           />
                           </>
                         }
                      />
           ...
           <Row
            ...
            rowContent={
                    <Column
                    classValue="col-6"
                    colContent={<Label labelContent="Supplemental Questions:" />}
                    />
                }
           />
           <Row
                    ...
                    <>
                      {isVehiceMaintenanceProgramChecked && (
                        <Form.Control
                          as={"textarea"}
                          rows={3}
                          onChange={(e) => setVehicleMaintenanceDescription(e.target.value)}
                          disabled={!isEditing}
                 ...
                 colContent={
                    <>
                        {isVehicleCustomizedChecked && (
                             ...
                             as={"textarea"}
                             rows={3}
                             onChange={(e) => setVehicleCustomizedDescription(e.target.value)}
                             disabled={!isEditing}
                ...
                <Column
                 ...
                 colContent={
                    <Form.Check
                        type="switch"
                        id="custom-switch"
                        onChange={(e) => setIsVehicleOwnedProspect(e.target.checked)}
                        disabled={!isEditing}
                ...
                <Column
                 ...
                 colContent={
                    <Form.Check
                        type="switch"
                        id="custom-switch"
                        onChange={(e) => setIsDeclinedCanceledNonRenewed(e.target.checked)}
                        disabled={!isEditing}
                ...
                <Column
                 ...
                 colContent={
                    <Form.Check
                        type="switch"
                        id="custom-switch"
                        onChange={(e) => setIsProspectLoss(e.target.checked)}
                        disabled={!isEditing}
                ...
                <Column
                 ...
                 colContent={
                    <Form.Check
                        type="switch"
                        id="custom-switch"
                        onChange={(e) => setIsVehicleUsedTowing(e.target.checked)}
                        disabled={!isEditing}
                ...
                <Column
                 ...
                 colContent={
                    ...
                    <DatePicker
                     ...
                     onChange={(date) => setExpirationOfAuto(date)}
                     disabled={!isEditing}
                    />
                    </>
                 }
