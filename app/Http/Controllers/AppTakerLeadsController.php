    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()){
            if(Cache::has('apptaker_leads')){
                ...
                $data = $query->select('id', 'company_name', 'tel_num', 'state_abbr', 'website_originated', 'created_at', 'disposition_id')->get();
            ...
            return DataTables::of($data)->addIndexColumn()
                ...
                ->addColumn('dispositions', function ($data){
                    ...
                    return '<select name="dispositions" id="dispositionDrodown" class="form-control select2">'.$options.'</select>';
                })
                ->rawColumns(['company_name_action', 'dispositions'])
                ->make(true);
        }
        return view('leads.apptaker_leads.index', compact('timezones', 'sites', 'states'));
